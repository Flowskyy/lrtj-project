...
import { ShoppingBag, Bell, ChevronLeft, User, Lock, Mail, Package, Gift, Home } from "lucide-react"
        ...
        <SidebarHeader className="py-4 px-3 flex items-center justify-center group-data-[state=collapsed]:py-6 overflow-hidden">
          <div className="flex items-center justify-center w-full h-auto">
            <Image
              ...
              width={40}
              height={40}
              className="object-contain hidden md:block group-data-[state=expanded]:hidden"
              priority
            />
            <Image
              src="/logo-lrtj.png"
              ...
              width={140}
              height={70}
              className="object-contain block md:group-data-[state=collapsed]:hidden max-w-full"
              priority
            />
          </div>
        </SidebarHeader>
        ...
        <header className="sticky top-0 z-30 flex h-14 shrink-0 items-center gap-2 bg-white px-4 shadow-[0_0.5px_0_rgba(0,0,0,0.1)]">
                ...
                <React.Fragment key={`${item}-${index}`}>
                  <BreadcrumbItem key={item}>
                    {index === pageMeta.breadcrumb.length - 1 ? (
                      <BreadcrumbPage className="text-primary font-semibold">{item}</BreadcrumbPage>
                    ) : (
                      <BreadcrumbLink 
                        href={
                          item === "Dashboard" 
          ...
          <div className="ml-auto flex items-center gap-3">
            <Button variant="ghost" size="icon" className="relative h-9 w-9">
              ...
              <Badge className="absolute -top-1 -right-1 h-4 w-4 flex items-center justify-center bg-primary text-white text-[9px] p-0 rounded-full">
                0
              </Badge>
            </Button>
            <DropdownMenu>
